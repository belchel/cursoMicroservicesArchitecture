...
namespace ProductCatalogApi.Data
{
    public class CatalogSeed
    {
        ...
        public static async Task SeedAsync(CatalogContext context)
        {
            ...
            {
                context.CatalogItems.AddRange(GetPreconfiguratedCatalogItems());
                await context.SaveChangesAsync();
            }
        }

        static IEnumerable<CatalogBrand> GetPreconfiguratedCatalogBrands()
        {
            return new List<CatalogBrand>()
            {
                new CatalogBrand(){ Brand = "Adidas"},
                new CatalogBrand(){ Brand = "Puma"},
                new CatalogBrand(){ Brand = "Rainha"}
            };
        }

        static IEnumerable<CatalogType> GetPreconfiguratedCatalogTypes()
        {
            return new List<CatalogType>()
            {
                new CatalogType(){ Type = "Running"},
                new CatalogType(){ Type = "Volley"},
                new CatalogType(){ Type = "Tennis"}
            };
        }

        static IEnumerable<CatalogItem> GetPreconfiguratedCatalogItems()
        {
